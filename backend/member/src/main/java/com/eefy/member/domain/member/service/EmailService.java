...
import com.eefy.member.domain.member.persistence.EmailRedisRepository;
...
public class EmailService {
    ...
    public ResponseEntity<String> confirmEmail(EmailConfirmRequest emailConfirmRequest) {
        ...
        if (!emailRedisRepository.existsById(member.getEmail())) {
            log.error("이메일 인증번호 확인 오류");
            throw new IllegalStateException("이메일 인증번호 확인 오류");
        }

        emailRedisRepository.deleteById(email);
    ...
    private MimeMessage createEmailForm(String code, String email) throws MessagingException {
        ...
        message.setFrom(username); //보내는 이메일
